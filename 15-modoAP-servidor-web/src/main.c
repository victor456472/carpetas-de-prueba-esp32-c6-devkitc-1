...
esp_err_t root_handler(httpd_req_t *req) {
    /*
    la siguiente linea establece el tipo de contenido que el servidor
    enviara como respuesta. en este caso, "text/html" indica que el 
    contenido enviado será código HTML que el navegador del cliente 
    podrá renderizar como una pagina web. Esto se realiza modificando
    el encabezado content-type de la respuesta.
    */
    httpd_resp_set_type(req, "text/html");
    /*
    la siguiente linea envía la respuesta HTTP al cliente. 

    tiene como parametro los siguientes campos:
    req: la solicitud HTTP que se está manejando
    html_form: el código HTML que se enviará como respuesta
    HTTPD_RESP_USE_STRLEN: especifica que se usará la longitud del
    contenido (el HTML), la cual se calculará automaticamente a 
    partir del terminador nulo (\0)
    */
